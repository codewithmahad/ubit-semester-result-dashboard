    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden print-card">
      ...
      <div className="bg-gray-50 border-b border-gray-200 px-5 py-4 flex items-center justify-between">
        <div>
          ...
          <h3 className="text-sm font-bold text-gray-900 mt-1">{semester.semesterName}</h3>
        </div>
      ...
      <div className="divide-y divide-gray-100">
        <div className="px-5 py-2 flex items-center bg-white text-[10px] font-semibold text-gray-400 uppercase tracking-wide">
          <div className="flex-1">Course</div>
          <div className="w-16 text-center">Marks</div>
          <div className="w-16 text-center">Grade</div>
          <div className="w-16 text-right">GP</div>
        </div>
        {semester.courses.map((course) => (
          <div key={course.courseCode} className="px-5 py-3 flex items-center bg-white hover:bg-gray-50/50 transition-colors">
            <div className="flex-1 min-w-0 pr-4">
              <div className="flex items-center gap-2 mb-0.5">
                <span className="font-mono text-[11px] font-bold text-gray-500">{course.courseCode}</span>
                <span className="text-gray-300">·</span>
                <span className="text-[10px] bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded font-medium">{course.creditHours} cr</span>
              </div>
              <p className="text-sm font-medium text-gray-900 truncate">{course.courseName}</p>
            </div>
            
            <div className="w-16 text-center">
              <span className="text-sm font-semibold tabular-nums text-gray-900">{course.marks}</span>
            </div>
            
            <div className="w-16 text-center flex justify-center">
              <span className={`inline-flex items-center justify-center w-8 h-6 rounded text-xs font-bold border ${gradeColor(course.grade)}`}>
                {course.grade}
              </span>
            </div>
            
            <div className="w-16 text-right">
              <span className="text-sm font-medium tabular-nums text-gray-600">{course.gradePoint.toFixed(1)}</span>
            </div>
          </div>
        ))}
      </div>
