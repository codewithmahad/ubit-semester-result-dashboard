...
export function ResultTable({ data, allSemData }: ResultTableProps) {
    ...
    function thSticky(id: string): React.CSSProperties {
        ...
        if (id === "name") return { position: "sticky", left: 220, zIndex: 40, width: 190, minWidth: 190, background: C.navy, boxShadow: "2px 0 0 #1E293B" }
        return {}
    }
    function tdSticky(id: string, bg: string): React.CSSProperties {
        if (id === "rank") return { position: "sticky", left: 0, zIndex: 20, width: 90, minWidth: 90, background: bg || "#fff" }
        if (id === "roll") return { position: "sticky", left: 90, zIndex: 20, width: 130, minWidth: 130, background: bg || "#fff", boxShadow: "1px 0 0 #F1F5F9" }
        if (id === "name") return { position: "sticky", left: 220, zIndex: 20, width: 190, minWidth: 190, background: bg || "#fff", boxShadow: "2px 0 0 #F1F5F9" }
        return {}
    }
                                        ...
                                        >
                                            {row.getVisibleCells().map(cell => (
                                                <td
                                                    key={cell.id}
                                                    style={{
                                                        ...tdSticky(cell.column.id, bg === "transparent" ? "#fff" : bg),
                                                        padding: "12px 16px",
