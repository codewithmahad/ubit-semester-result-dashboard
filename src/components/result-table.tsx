...
import {
  calculateRankings,
  calculateCGPARankings,
} from "@/lib/utils/academic-math";
import {
  Download,
  Search,
  CheckCircle2,
...
import {
  ...
  DiamondIcon,
  ShieldIcon,
} from "@/components/ui/medal-badge";
import {
  Th,
  thStickyClassResult,
  tdStickyClassResult,
...
export function ResultTable({ data, allSemData, tabsList }: ResultTableProps) {
  ...
  const [selectedStudent, setSelectedStudent] = useState<StudentRanking | null>(
    null,
  );
  ...
  const rankedStudents = useMemo(
    () => calculateRankings(data.courses, data.students),
    [data],
  );
  ...
  const cumulativeRankMap = useMemo(() => {
    ...
    calculateCGPARankings(allSemData).forEach((r) => {
      if (r.rank !== undefined) map.set(r.roll, r.rank);
    });
    return map;
  }, [allSemData]);
  ...
  function sgpaChipClass(sgpa: number, cr: number | undefined) {
    ...
    if (sgpa >= 2.0)
      return "bg-slate-100 text-ubit-ink border border-slate-300";
  ...
  const columns = useMemo<ColumnDef<StudentRanking>[]>(() => {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex flex-col justify-center">
            ...
            <span className="text-[9px] sm:text-[10px] font-semibold text-slate-400 mt-0.5 leading-none">
              {row.original.name}
            </span>
          </div>
        ),
    ...
    const courseCols = data.courses.map(
      (course): ColumnDef<StudentRanking> => ({
        id: course.code,
        ...
        header: () => (
          <div className="flex flex-col items-center text-center leading-tight">
            ...
            <span
              className="text-[8px] sm:text-[9px] font-medium uppercase tracking-widest mt-0.5"
              style={{ color: "#94A3B8" }}
            >
        ...
        cell: ({ getValue }) => {
          const res = getValue() as
            { marks: number; grade: string } | undefined;
          if (!res)
            return (
          ...
          return (
            <div className="flex flex-col items-center text-center w-full">
              <span className="text-[12px] sm:text-[14px] tabular-nums font-bold leading-tight text-ubit-ink">
                {res.marks}
              </span>
              <span
                className={`text-[9px] sm:text-[10px] tracking-wide mt-0.5 font-bold ${gradeColor(res.grade)}`}
              >
                {res.grade}
              </span>
            </div>
    ...
    const endCols: ColumnDef<StudentRanking>[] = [
      ...
      },
      {
        ...
        header: () => <Th>Status</Th>,
        cell: ({ row }) => {
          const failCount = Object.values(row.original.results).filter(
            (r) => r.grade === "F" || r.gradePoint === 0,
          ).length;
          if (failCount > 0)
            return (
              <div className="flex justify-center items-center gap-1.5 w-full">
                <XCircle className="w-3.5 h-3.5 shrink-0 text-ubit-faint" />
                <span className="text-[12px] font-semibold text-ubit-muted">
                  Fail <span className="text-ubit-faint">({failCount})</span>
                </span>
              </div>
            );
          return (
  ...
  return (
    <div className="space-y-3 relative z-10 w-full overflow-hidden">
      ...
      >
        ...
        <div
          className="overflow-auto max-h-[70vh] w-full styled-scrollbar rounded-2xl"
                ...
                <tr key={hg.id} className="bg-ubit-navy">
                  {hg.headers.map((h) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`flex items-center ${["roll", "name"].includes(h.id) ? "justify-start" : "justify-center"} gap-1 w-full`}
                        >
                          {flexRender(
                          ...
                          {sortable && (
                            <span
                              style={{
                                opacity: h.column.getIsSorted() ? 1 : 0.25,
                    ...
                    <tr
                      ...
                      onClick={() => {
                        ...
                        saveToRecentViews({
                          ...
                          shift: data.batch.includes("Morning")
                            ? "Morning"
                            : "Evening",
                        });
                      }}
          ...
          <div className="flex items-center gap-3">
            {MEDAL_LEGEND.map((m) => (
              <div key={m.label} className="flex items-center gap-1.5">
                <span
                  className={`${m.colorClass} rounded px-1 py-0.5 flex items-center`}
                ></span>
                <span className="text-[10px] font-bold uppercase tracking-widest text-ubit-faint">
                  {m.label}
                </span>
              </div>
            ))}
      ...
      <StudentModal
        ...
        cumulativeRank={
          selectedStudent
            ? (cumulativeRankMap.get(selectedStudent.roll) ?? null)
            : null
        }
      />
    </div>
